...
export default function Post({
  ...
  useEffect(() => {
    if (post.post.content.startsWith('[this')) return;

    checkIfLiked({ loggedUserId: loggedUser.id, postId: post.post.id })
      .then((isLiked) => setLiked(isLiked))
      .catch((err) => console.error('Failed to check like status:', err));
  }, [loggedUser.id, post.post.id, post.post.content]);
  ...
  const handleLike = () => {
    ...
    likeAction({ loggedUserId: loggedUser.id, postId: post.post.id }).catch(() => {
      setLiked(!newLikedState);
  ...
  const gridImages: GridImage[] =
    post.images?.map((img, index) => ({
      id: `post-${post.post.id}-image-${index}`,
      url: img.publicUrl || '/placeholder.svg',
    })) || [];

  const isDeleted = post.post.content.startsWith('[this');
          ...
          <div className="min-w-0">
            <div className="flex justify-between items-start mb-1">
              <div className="flex flex-wrap items-center gap-x-1 min-w-0 pr-2">
                ...
                <span className="text-muted-foreground text-sm truncate hidden sm:inline hover:text-primary transition-colors">
                  {new Date(post.post.createdAt).toLocaleDateString()} at{' '}
              ...
              <Link href={`/post/${post.post.id}`} className="shrink-0">
            ...
            {!isDeleted && (
              <div className="flex justify-between items-center mt-2">
                ...
                <ShareButton loggedUserId={loggedUser.id} postUserId={post.user.id} postId={post.post.id} />
              </div>
            )}
          </div>
        </div>
